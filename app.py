...
def user_data_path(email, suffix):
    safe_email = email.replace("@", "_at_").replace(".", "_")
    return os.path.join(INSTANCE_FOLDER, f"{safe_email}_{suffix}")
...
def upload():
    if request.method == "POST":
        ...
        try:
            ...
            session["dashboard_json_path"] = dashboard_json_path
            session["prediction_csv_path"] = csv_path
            session["live_csv_path"] = live_csv_path
...
@login_required
def dashboard():
    dashboard_json_path = session.get("dashboard_json_path")
    data = load_json(dashboard_json_path, default=None)
...
def toggle_attack():
    ...
    return jsonify({"attack_mode": SIMULATION_STATE["attack_mode"]})
...
def api_mobile_live():
    try:
        ...
        if ENABLE_SNIFFER and not sniffer_error and not live_df.empty:
            ...
            alert_df = original_live_df.copy()
            alert_df["predicted_class"] = full_result_df["predicted_class"].values
            alert_df["confidence"] = full_result_df["confidence"].values
            alert_df["threat_status"] = full_result_df["threat_status"].values
...
@login_required
def download_csv():
    file_path = session.get("prediction_csv_path")
